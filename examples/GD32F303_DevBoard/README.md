...
ADC0 底层通过双快照地址发布 PA3 输出电流、PA4 输出电压、PA5 输入电压的硬件 8 倍平均 raw，不在 BSP 内进行软件平均或物理量换算；ADC1 使用长度为 1 的规则序列，每次 EOC 中断保存当前相 raw 并把 Rank0 切换到下一相，读取结果允许来自相邻轮转。L3 `ADCMeasurement_*` 校验 ADC0 快照令牌后，使用 3.3 V/4096 量化比例换算实际值：输出电压增益 29.75、输入电压增益 5.29、输出电流增益 20 A/V，相电流按 `(Vadc - 1.1 V) * 8 A/V` 计算并保留负值。
